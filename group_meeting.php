...
function renderCharts(d){
  ...
  if(msEl){
    ...
    charts.push(new Chart(msEl.getContext('2d'),{type:'bar',data:{labels:msLabels,datasets:CO_KEYS.map(k=>({label:CO_LABELS[k],data:msLabels.map((_,idx)=>{const ms=cos[k].monthlySales||[];return Math.round((ms[idx]||0)/10000)}),backgroundColor:CO_COLORS[k]}))},options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{labels:{color:'#f6f1e6',font:{weight:'bold'}}},tooltip:{callbacks:{label:c=>{const ds=c.dataset;const total=ds.data.reduce((a,b)=>a+b,0);const v=c.parsed.y||0;return` ${c.dataset.label}: ${v} 萬 (佔比 ${v>0?(v/total*100).toFixed(1):0}%)`}}}},scales:{x:{stacked:true,grid:{display:false},ticks:{color:'#a9a39a'}},y:{stacked:true,grid:{color:'rgba(255,255,255,.05)'},ticks:{color:'#a9a39a',callback:v=>v+'萬'}}}}}));
  }
}
